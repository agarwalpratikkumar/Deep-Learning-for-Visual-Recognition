...
class GAN(nn.Module):
	...
	def model(self):
		...
		if self.resume:
			print('Inside resume GAN')
			#pdb.set_trace()
			netG.load_state_dict(torch.load(self.out_path + 'netG_epoch_{}.pth'.format(self.resume)))
			print('Generator')
			print(netG)
			netD.load_state_dict(torch.load(self.out_path + 'netD_epoch_{}.pth'.format(self.resume)))
			print('Discriminator')
			print(netD)
