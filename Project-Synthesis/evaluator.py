	...
	def train(self, dataloader, netG, netD):
		...
		for epoch in range(self.nm_epochs):
			for i, data in enumerate(dataloader, 0):
				...
				#pdb.set_trace()
				label = torch.full((real_cpu.shape[0],), self.real_label, 
										device=self.device)
				...
				# train with fake
				noise = torch.randn(real_cpu.shape[0], self.nz, 1, 1, device=self.device)
				fake = netG(noise)
				self.vector_form = fake.data.clone()
				...
				print('[{}/{}][{}/{}] Loss_D: {:.4f} Loss_G: {:.4f} D(x): {:.4f} D(G(z)): {:.4f} / {:.4f}'
					.format(epoch+1, self.nm_epochs, i, len(dataloader), errD.item(), errG.item(), D_x, D_G_z1, D_G_z2))
				
				if i % 5 == 0:
					print('value of i: ',i)
					vutils.save_image(real_cpu, self.out_path + 'real_samples.png', normalize=True)
					fake = netG(fixed_noise)
					vutils.save_image(fake.detach(), self.out_path + 'fake_samples_epoch_{}.png'.format(epoch+1), normalize=True)
				
			if epoch % 3 == 0:
				print('epoch no:',epoch)
				torch.save(netG.state_dict(), self.model_path + 'netG_epoch_{}.pth'.format(epoch+1))
				torch.save(netD.state_dict(), self.model_path + 'netD_epoch_{}.pth'.format(epoch+1))
